...
export function BookmarksModal({ slug, volumeId, onClose }: BookmarksModalProps) {
  ...
  const relevant = useMemo(() => {
    ...
    const filtered = slug && volumeId
      ? all.filter((b) => b.slug === slug && b.volumeId === volumeId)
      : all;
  ...
  return (
    <div
      className="fixed inset-0 z-[85] flex items-end justify-center bg-slate-950/80 p-0 backdrop-blur-sm md:items-center md:p-4"
      onClick={onClose}
    >
      <div
        className="flex max-h-[80vh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-slate-800 bg-slate-900 shadow-2xl md:rounded-3xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
          <h2 className="flex items-center gap-2 text-lg font-bold text-slate-100">
            <Bookmark size={20} className="text-amber-400" />
            我的书签 <span className="text-sm font-normal text-slate-500">({relevant.length})</span>
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {relevant.length === 0 ? (
            <p className="py-10 text-center text-sm text-slate-500">暂无书签，在 entry 旁点 📖 图标添加</p>
          ) : (
            <ul className="space-y-2">
              {relevant.map((bm) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => removeBookmark(buildEntryKey(bm.slug, bm.volumeId, bm.entryIndex))}
